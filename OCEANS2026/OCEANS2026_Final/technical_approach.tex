...
Depth is estimated using the Metric-Outdoor variant of Depth Anything V2, fine-tuned from the foundation model on synthetic Virtual KITTI data~\cite{gaidon2016virtualworldsproxymultiobject}. To correct scale bias from road-centric pre-training without retraining on scarce maritime data, we apply a single global scaling factor calibrated once against radar ground truth from the WaterScenes dataset~\cite{yao2024waterscenesmultitask4dradarcamera}:
\begin{equation}
  d_\mathrm{cal} = s_{cal} \cdot d_\mathrm{raw}, \qquad s_{cal} = 0.4,
  \label{eq:depth_cal}
\end{equation}
...
To mitigate monocular depth noise, we extend the SORT tracker to a 9-D Kalman state that tracks bounding-box dynamics and metric depth jointly:
\begin{equation}
  \mathbf{x} = [c_x, c_y, s, r, d, \dot{c}_x, \dot{c}_y, \dot{s}, \dot{d}]^\top,
  \label{eq:kalman_state}
\end{equation}
where $(c_x, c_y)$, $s$, and $r$ denote the bounding-box center, scale (area), and aspect ratio; $d$ is the calibrated metric depth; and dotted variables represent their respective rates of change. The filter optimally balances depth observation noise against a constant-velocity process model, yielding smoothed estimates of the closing velocity,
\begin{equation}
  v_{\mathrm{closing}} = -\dot{d} \cdot f_{\mathrm{fps}},
  \label{eq:closing_velocity}
\end{equation}
where $f_{\mathrm{fps}}$ is the system frame rate. Whenever $v_\mathrm{closing}$ exceeds 0.1\,m/s, this also yields the Time-to-Collision,
\begin{equation}
  TTC = d / v_{\mathrm{closing}},
  \label{eq:ttc}
\end{equation}
computed from the Kalman-filtered depth rather than the noisy per-frame observation. Both quantities feed directly into the risk model in Eq.~(\ref{eq:risk}) without requiring post-hoc exponential moving averages or windowing.

This formulation improves on post-hoc velocity estimation, such as finite differencing or regression over a depth history buffer, in three respects. The filter balances observation noise against process dynamics optimally, automatically down-weighting an unreliable depth reading rather than trusting it outright. The velocity $\dot{d}$ is estimated jointly with the rest of the state, so trajectories come out smooth without a separate moving-average or windowing step. And during a temporary loss of detection, within the track's grace window described below, the filter keeps propagating depth forward under the motion model instead of dropping the estimate, so a brief occlusion degrades gracefully rather than resetting the velocity to zero.
...
Based on the Responsibility-Sensitive Safety (RSS) framework~\cite{shalevshwartz2018formalmodelsafescalable}, we define a virtual forward collision corridor of width $W_\mathrm{boat} + 2M_\mathrm{lat}$ centered on the ASV heading, where $W_\mathrm{boat}$\ is the ASV beam width and $M_\mathrm{lat}$\ is the lateral safety margin. For an obstacle with forward distance $d_\mathrm{fwd}$, the lateral distance is $d_\mathrm{lat} = \ell \cdot d_\mathrm{fwd} \cdot \tan(\theta_\mathrm{HFOV}/2)$, and its excess beyond the corridor is
\begin{equation}
  e_\mathrm{lat} = \max\bigl(0,\ |d_\mathrm{lat}| - (W_\mathrm{boat}/2+M_\mathrm{lat})\bigr).
  \label{eq:lateral_excess}
\end{equation}
With this, the static risk baseline is
\begin{equation}
  R_\mathrm{base} = w_\mathrm{class} \cdot \exp\!\left(-d_\mathrm{fwd}/D_\mathrm{safe}\right) \cdot \exp\!\left(-e_\mathrm{lat}/L_\mathrm{safe}\right),
  \label{eq:risk_base}
\end{equation}
